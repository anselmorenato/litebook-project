...
#This is the source file of LiteBook Linux version 1.4
...
#add EnableSidebarPreview in option_dialog
#update OptionDialog to align with dir sidebar, which include hotkey
...
#fix a bug in optiondialog
#fix a bug in HTML filter
...
#            GlobalConfig['LastDir']=GlobalConfig['LastDir'].decode('big5')
    ...
    def OnChar(self, event):
        usedKeys=(wx.WXK_DOWN,wx.WXK_RIGHT,wx.WXK_UP,wx.WXK_LEFT,wx.WXK_PAGEDOWN,wx.WXK_SPACE,wx.WXK_PAGEUP,wx.WXK_HOME,wx.WXK_END,12,15,20,70,72,74,wx.WXK_ESCAPE,wx.WXK_TAB,wx.WXK_RETURN)
        CTRL=2
        ALT=1
        SHIFT=4
        key=event.GetKeyCode()
    ...
    def DirSideBarReload(self):
        ...
        for filename in file_list:
            ...
            if os.path.isdir(current_path)== False:
                if os.path.splitext(current_path)[1].lower()=='.txt' :
                    ...
                    if (os.path.splitext(current_path)[1].lower()=='.htm' or os.path.splitext(current_path)[1].lower()=='.html'):
                        ...
                        if os.path.splitext(current_path)[1].lower()=='.zip' :
                            ...
                            if os.path.splitext(current_path)[1].lower()=='.rar':
                                ...
                                self.itemDataMap[index]=(filename,os.path.getsize(current_path),Date_str)
                                 ...
                                 if os.path.splitext(current_path)[1].lower()=='.jar' :
                                     ...
                                     if os.path.splitext(current_path)[1].lower()=='.umd':
