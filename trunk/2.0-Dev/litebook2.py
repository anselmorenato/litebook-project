...
def jarfile_decode(infile):
    ...
    cache_dir=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\cache"
    fp=open(infile,'r')
    s=fp.read()
    m=hashlib.md5()
    m.update(s)
  ...
  #      self.text_ctrl_1.Refresh()
    ...
    def Menu112(self,evt):
        ...
        if dlg.ShowModal()==wx.ID_YES:
            flist=glob.glob(os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\cache\\*")
            suc=True
            for f in flist:
                try:
                    os.remove(f)
