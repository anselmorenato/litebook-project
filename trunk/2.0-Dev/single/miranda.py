...
##	print ''
	...
	for x in range(4):#try 3 times (because 1st recv packet is a discovery sent by self
		try:
			r = hp.parseSSDPInfo(hp.listen(1024,server),False,False)
			if r == True:
				return True
		except Exception, e:
			return False
...
##					print "Are you sure you've specified the correct action?"
				...
				print "sendArgs",sendArgs
				print hostInfo['name'],'---',fullServiceName,'---',controlURL,'---',actionName,
                #sendArgs {'NewPortMappingDescription': ('test-1', 'string'), 'NewLeaseDuration': ('0', 'ui4'), 'NewInternalClient': ('10.10.10.248', 'string'), 'NewEnabled': ('1', 'boolean'), 'NewExternalPort': ('80', 'ui2'), 'NewRemoteHost': ('', 'string'), 'NewProtocol': ('UDP', 'string'), 'NewInternalPort': ('1037', 'ui2')}
				soapResponse = hp.sendSOAP(hostInfo['name'],fullServiceName,controlURL,actionName,sendArgs)
				print 'retTags',retTags
...
#add_port_mapping
def addPortMapping(hp,index,mapName,mapInternalIP,mapInternalPort,
					 mapExternalPort,mapProto):
	"""
	add_port_mapping,return True/False as the result
	map_xxx are all strings
	"""
	#Send SOAP requests
	inArgCounter = 0

	deviceName = 'WANConnectionDevice'
	serviceName = 'WANIPConnection'
	actionName = 'AddPortMapping'
	hostInfo = hp.ENUM_HOSTS[index]
	actionArgs = False
	sendArgs = {'NewPortMappingDescription': (mapName, 'string'),
				'NewLeaseDuration': ('0', 'ui4'),
				'NewInternalClient': (mapInternalIP, 'string'),
				'NewEnabled': ('1', 'boolean'),
				'NewExternalPort': (mapExternalPort, 'ui2'),
				'NewRemoteHost': ('', 'string'),
				'NewProtocol': (mapProto, 'string'),
				'NewInternalPort': (mapInternalPort, 'ui2')}
	retTags = []
	controlURL = False
	fullServiceName = False
...
#remove_port_mapping
def delPortMapping(hp,index,mapExternalPort,mapProto):
	"""
	add_port_mapping,return True/False as the result
	map_xxx are all strings
	"""
	...
	try:
	   hostInfo = hp.ENUM_HOSTS[index]
		...
		return False
	actionArgs = False
	sendArgs = {'NewPortMappingDescription': ('', 'string'),
				'NewLeaseDuration': ('0', 'ui4'),
