...
# - rarfile
# - unrar2(modified)
# - wxpython 2.8.10.1 unicode or later

# generated by wxGlade 0.6.3 on Tue May 05 22:48:24 2009

#2012-07-03 fix a bug of GenCatalog which produce duplicate chapters
#2012-01-16 enhance GenCatalog
#2011-11-19 fix more bugs of reloading toolbar
#2011-11-16 fix a bug of reloading the toolbar, the show_mode button doesn't display correctly
#2011-11-12 fix a bug of searching for book
#2011-09-19 add error exception when create web server fail
#2011-09-15 add resizable toolbar
#2011-09-15 add "first page" when generate catalog
#2011-09-14 add mulu siderbar
#2011-09-14 fix issue 52
#2011-09-14 fix a bug of epub file decoding
#2011-09-14 enhance epub dialog, so the saved filename will chage according to the change of title
#2011-05-10 remove self.text_ctrl_1.SetSelection from the code, which is no longer available from liteview
#2011-05-08 add mDNS for auto-discovery of shared books
#2011-04-12 change cache dir and sharebook dir to USERPROFILE\litebook
#2011-04-02 add web server
#2011-03-22 add epub file generation
#2011-03-20 update epub_decoe() and gcepub to fix filename sort problem
#2011-03-06 add message dialog when loading file
#2011-03-06 add cache to epub and jar decoding
#2011-03-06 add auto generate catalog
...
def readConfigFile():
    ...
    try:
        ...
        try:
            ffp=codecs.open(os.path.dirname(os.path.abspath(sys.argv[0]).decode('gbk'))+u"\\litebook.ini",encoding='utf-8',mode='r')
            GlobalConfig['ConfigDir']=os.path.dirname(os.path.abspath(sys.argv[0])).decode('gbk')
            config.readfp(ffp)
        except:
            ...
            #GlobalConfig['BackgroundDir']=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\background"
            OpenedFileList=[]
...
##    except:
##        GlobalConfig['backgroundimg']=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\background"+"\\default.jpg"
##
##    try:
...
##    try:
##        GlobalConfig['ServerAddr']=config.get('settings','ServerAddr')
##    except:
##        GlobalConfig['ServerAddr']=''
    ...
    if GlobalConfig['LastDir'].strip()=='' or os.path.isdir(GlobalConfig['LastDir'])==False:
        GlobalConfig['LastDir']=os.path.dirname(os.path.abspath(sys.argv[0]))
...
##    config.set('settings','ServerAddr',unicode(GlobalConfig['ServerAddr']))
...
def InstallDefaultConfig():
    ...
    fname=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\defaultconfig.ini"
    config=MyConfig()
    try:
...
#        self.Bind(wx.EVT_LIST_ITEM_SELECTED, self.OnItemSelected,self.list_ctrl_1)
        ...
        #set Sshow mode, this has to be here, otherwise system won't load if showmode is book or vbook
...
##        # end wxGlade
##        if mode=="help":fname=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\LiteBook_Readme.txt"
##        else:
##            fname=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\LiteBook_WhatsNew.txt"
##        try:
##            f=open(fname,'r')
##            t_buff=f.read()
##        except:
...
##            dlg.Destroy()
##            return False
##        coding=DetectFileCoding(os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u"\\LiteBook_Readme.txt")
##        if coding=='error': return False
##        utext=AnyToUnicode(t_buff,coding)
...
##            weblist.append(row[0])
        flist=glob.glob(os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+"\\plugin\\*.py")
        for f in flist:
    ...
    def __set_properties(self):
        ...
        if GlobalConfig['lastweb']=='' or not os.path.exists(os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+"\\plugin\\"+GlobalConfig['lastweb']+'.py'):
...
class web_search_result_dialog(wx.Dialog):
    def __init__(self, parent,sitename,keyword):
        ...
        if sitename<>u'搜索所有网站':
            ...
            sr=[]
            flist=glob.glob(os.path.dirname(os.path.abspath(sys.argv[0]))+"\\plugin\\*.py")
            #plugin_list=[]
...
class cnsort:
    ...
    # 建立拼音辞典
        self.dic_py = dict()
        try:
            f_py = open(os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+'\\py.dat',"r")
            f_bh = open(os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+'\\bh.dat',"r")
        except:
            return None
...
##        self.label_webaddr = wx.StaticText(self.notebook_1_pane_2, -1, u"Web服务器地址：")
...
##        self.text_ctrl_7.SetToolTipString(u"*代表绑定本机所有地址")
...
##        sizer_web = wx.BoxSizer(wx.HORIZONTAL)
...
##        sizer_web.Add(self.label_webaddr, 0, wx.ALIGN_CENTER_VERTICAL, 0)
##        sizer_web.Add(self.text_ctrl_7, 0, 0, 0)
##        sizer_weball.Add(sizer_web, 0, wx.EXPAND, 0)
...
##    def OnAutoSelInt(self,evt):
##        self.combo_box_MDNS.SetStringSelection(self.auto_int)
    ...
    def OnOK(self,event):
        ...
        if self.combo_box_4.GetSelection()==0:
            if self.text_ctrl_3.bg_img_path<>None and self.text_ctrl_3.bg_img_path<>'':
                if os.path.dirname(AnyToUnicode(self.text_ctrl_3.bg_img_path))==os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u'\\background':
...
##        svr_ip=self.text_ctrl_7.GetValue().strip()
##        if svr_ip=='*':
##            GlobalConfig['ServerAddr']=''
##        else:
##            try:
##                socket.inet_aton(svr_ip)
##                GlobalConfig['ServerAddr']=svr_ip
##            except:
##                GlobalConfig['ServerAddr']=''
    ...
    def OnSaveTheme(self,event):
        ...
        if r['backgroundimg']<>None and r['backgroundimg']<>'':
            if os.path.dirname(r['backgroundimg'])==os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u'\\background':
...
#        data.SetColour(GlobalConfig['CurFColor'])
    ...
    def OnSelectBG(self,evt):
        ...
        if defaultpath=='':
            defaultpath=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u'\\background'
        ...
        if rpath<>None and rpath<>'':
            if os.path.dirname(AnyToUnicode(rpath))==os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+u'\\background':
    ...
    def OnExportTheme(self,evt):
        ...
        if len(rlist)>0:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                try:
                    tmp_ini_name=os.environ['TMP']+'\\_tmp_ltb_thm_export.ini'
                ...
                export_file.write(os.environ['TMP']+'\\_tmp_ltb_thm_export.ini','LB_Display_theme.ini')
                for img in img_file_list:
                    ...
                    if fpath<>'' and fpath<>None:
                        if img.find('\\')==-1:
                            fpath=os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])).encode('GBK'))+'\\background\\'+img
                        try:
            ...
            try:
                ...
                for fname in fnamelist:
                    if fname.find("..")<>-1 or fname.find("\\")<>-1:
                        raise
                importf.extractall(os.environ['TMP'])
                config=MyConfig()
                ffp=codecs.open(os.environ['TMP']+'\\LB_Display_theme.ini',encoding='utf-8',mode='r')
                config.readfp(ffp)
            ...
            for r in rlist:
                ...
                try:
                    ...
                    if f[9]<>'None' and f[9]<>'':
                        l['backgroundimg']=f[9]
                        if f[9] in fnamelist:
                            shutil.copyfile(os.environ['TMP']+"\\"+f[9],os.path.dirname(AnyToUnicode(os.path.abspath(sys.argv[0])))+'\\background\\'+f[9])
                        else:
...
##        SqlCon.execute(sqlstr)
    ...
    #app = wx.PySimpleApp(True,os.environ['USERPROFILE'].decode('gbk')+u"\\litebook\\litebook.log")
    app = wx.PySimpleApp()
    fname=None
    if len(sys.argv)>1:
