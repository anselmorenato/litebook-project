...
def GetBook(url,bkname='',win=None,evt=None,useproxy=False,proxyserver='',proxyport=0,proxyuser='',proxypass='',concurrent=10):
    ...
    if index_page==None: return None
    mybip=DBIP()
    ...
    while i<ccount:
        ...
        while x<n:
            ...
            x+=1
        while m<n:
            tlist=DThread(url[:-10]+mybip.rlist[(i+m)],m,useproxy,proxyserver,proxyport,proxyuser,proxypass)
            m+=1
        isfinished=False
        x=0
...
class DThread:
    ...
    def run(self):
        global tlock,tr
        tlock.acquire()
        tr[self.i]=myopen(self.url,None,self.useproxy,self.proxyserver,self.proxyport,self.proxyuser,self.proxypass)
        tlock.release()
