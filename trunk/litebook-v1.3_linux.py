...
#This is the source file of LiteBook Linux version 1.30
#require following 3rd party module:
# - chardet
# - rarfile
# - wxpython 2.8.10.1 and later
# - unrar linux version(from rarlab)
...
def readConfigFile():
    ...
    global GlobalConfig,OpenedFileList,BookMarkList,ThemeList,BookDB,LastOpenedFileList
    config = MyConfig()
    try:
        ffp=codecs.open(os.path.dirname(os.getcwd()+'/'+sys.argv[0])+u"/litebook.ini",encoding='utf-8',mode='r')
        GlobalConfig['ConfigDir']=os.path.abspath(os.path.dirname(os.getcwd()+'/'+sys.argv[0]))
    except:
        GlobalConfig['ConfigDir']=os.path.abspath(os.path.dirname(os.getcwd()+'/'+sys.argv[0]))
        GlobalConfig['LastDir']=os.path.abspath(os.path.dirname(os.getcwd()+'/'+sys.argv[0]))
        GlobalConfig['IconDir']=os.path.abspath(os.path.dirname(os.getcwd()+'/'+sys.argv[0])+u"/icon")
        OpenedFileList=[]
        LastOpenedFileList=[]
...
def UpdateOpenedFileList(filename,ftype,zfile=''):
    global OpenedFileList,GlobalConfig
    fi={}
    for f in OpenedFileList:
...
def unrar(rfile,filename):
    ...
    cmd_line=[]
    cmd_line.append(GlobalConfig['ConfigDir']+u"/unrar")
    cmd_line.append('x')
    cmd_line.append('-y')
    ...
    def __init__(self,parent,zipfilename):
        ...
        if os.path.splitext(zipfilename)[1].lower()== ".zip":
            ...
            for fname in zfile.namelist():
                zname_list.append(fname.decode('GBK'))
            self.AddTreeNodes(root,zname_list)
        else:
            ...
            for line in rfile.namelist():
                ...
                if rfile.getinfo(line).isdir(): line+=u"/"
...
#        self.text_ctrl_1.Bind(wx.EVT_MIDDLE_DOWN,self.MyMouseMDW)
    ...
    def __set_properties(self):
        ...
        # end wxGlade
        #self.text_ctrl_1.SetDefaultStyle(self.currentTextAttr)
        #self.text_ctrl_1.SetBackgroundColour('LIGHT BLUE')
...
#                                t_buff=file_handler.read()
    ...
    def GenList(self, zip):
        global current_file,GlobalConfig,current_file_list,current_zip_file,load_zip
        current_file_list=[]
#        print type(GlobalConfig['LastDir'])
#        try:
#            GlobalConfig['LastDir']=GlobalConfig['LastDir'].decode('gbk')
#        except:
#            GlobalConfig['LastDir']=GlobalConfig['LastDir'].decode('big5')
        if zip=='file':
    ...
    def OnChar(self, event):
        usedKeys=(wx.WXK_DOWN,wx.WXK_RIGHT,wx.WXK_UP,wx.WXK_LEFT,wx.WXK_PAGEDOWN,wx.WXK_SPACE,wx.WXK_PAGEUP,wx.WXK_HOME,wx.WXK_END,12,15,wx.WXK_ESCAPE,wx.WXK_TAB,wx.WXK_RETURN)
        CTRL=2
        ALT=1
        SHIFT=4
        key=event.GetKeyCode()
    ...
    def OpenLastFile(self, event):
        global LastOpenedFileList,OpenedFileList
        id=event.GetId()
        for f in LastOpenedFileList:
            if f['MenuID']==id:
                flist=[]
...
class MyOpenFileDialog(wx.Dialog,wx.lib.mixins.listctrl.ColumnSorterMixin):
    ...
    def OnItemActive(self,event):
        ...
        current_ext=os.path.splitext(current_path)[1].lower()
        if os.path.isdir(current_path)== True:
            GlobalConfig['LastDir']=current_path
            self.text_ctrl_2.SetValue(GlobalConfig['LastDir'])
            self.Reload()
        else:
...
class OptionDialog(wx.Dialog):
    ...
    def OnSaveTheme(self,event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for t in ThemeList:
                if t['name']==l['name']:
                    dlg = wx.MessageDialog(self, u'已经有叫这个名字的显示方案了，请重新取名！',u"错误！",wx.OK|wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return
            ...
            ThemeList.append(l)
            self.combo_box_1.Append(l['name'])
...
class HelpDialog(wx.Dialog):
    ...
    def __set_properties(self):
        ...
        # end wxGlade
#        try:
        f=open(os.path.dirname(os.getcwd()+'/'+sys.argv[0])+u"/LiteBook_Readme.txt",'r')
        t_buff=f.read()
#        except:
...
#            dlg.Destroy()
#            return False
        coding=DetectFileCoding(os.path.dirname(os.getcwd()+'/'+sys.argv[0])+u"/LiteBook_Readme.txt")
        if coding=='error': return False
        utext=AnyToUnicode(t_buff,coding)
