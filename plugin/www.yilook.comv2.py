...
def GetBook(url,bkname='',win=None,evt=None,useproxy=False,proxyserver='',proxyport=0,proxyuser='',proxypass='',concurrent=10):
    ...
    while i<ccount:
        ...
        while x<n:
            ...
            x+=1
        while m<n:
            tlist=DThread(mybip.srlist[(i+m)][1],m,useproxy,proxyserver,proxyport,proxyuser,proxypass)
            m+=1
        isfinished=False
        x=0
        while not isfinished:
            time.sleep(1)
            isfinished=isfull(tr,n)
        x=0
        while x<n:
            bb+=tr[x]
            x+=1
...
class DThread:
    def __init__(self,url,i,useproxy=False,proxyserver='',proxyport=0,proxyuser='',proxypass=''):
        self.url=url
    ...
    def run(self):
        global tlock,tr
        tlock.acquire()
        tr[self.i]=myopen(self.url,post_data=None,useproxy=self.useproxy,proxyserver=self.proxyserver,proxyport=self.proxyport,proxyuser=self.proxyuser,proxypass=self.proxypass)
        tlock.release()
