...
BookDB=[]
Ticking=True
Version='1.54 GLIBC2.7'
I_Version=1.54 # this is used to check updated version
...
# end of class ZipFileDialog
    ...
    def AddTreeNodes(self,parentItem,items,realitem=None):
        mylist=[]
        for item in items:
            if item.find('/') == -1:
                ...
                if realitem<>None:
                    self.tree_ctrl_1.SetPyData(t_item,realitem+item)
                else:
                    self.tree_ctrl_1.SetPyData(t_item,item)                
            else:
                if item.count('/')==1 and item.rfind('/')+1==item.__len__():
                    ...
                    self.AddTreeNodes(newItem, mylist,item)
